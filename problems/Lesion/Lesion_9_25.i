###############################################################################
# Lesion_9_25.i  -- Hard-lesion variant: mu_B=9 kPa, mu_L=25 kPa
# Cubit file mesh in x-z plane, Gaussian body force, Newmark time integration
#
# Same geometry and mesh as Lesion-DirBC; only lesion stiffness differs.
# The lesion is STIFFER than the background (c_s_L > c_s_B).
#
...
output_dir = '${data_dir}/Lesion_9_25/exodus'
...
mu_B = 9000.0   # Base (B)
mu_L = 25000.0    # Lesion (L) — soft lesion
...
# c_s_B = 3.0 m/s, c_s_L = 4.0 m/s
# At f_max = 1500 Hz: lambda_min = 2.0 mm (background governs — softer than lesion)
...
end_time = 20e-3                       # simulation end time (s) — just before first x-reflection enters imaging domain at ~16.7 ms
